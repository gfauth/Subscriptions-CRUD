...
namespace Observer.Settings
{
    internal sealed class Policies
    {
        ...
        {
            var policyCircuitBreaker = Policy.Handle<SqlException>().CircuitBreakerAsync(_circuitBreakerExceptionsAllowedCount, TimeSpan.FromMinutes(_circuitBreakerDuration));
            var policyRetry = Policy.Handle<SqlException>().WaitAndRetryAsync(retryCount: _retryCount, sleepDurationProvider: retryAttempt => TimeSpan.FromSeconds(Math.Pow(2, retryAttempt)));
